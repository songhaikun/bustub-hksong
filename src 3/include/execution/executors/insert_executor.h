...
public:
   ...
   * @param[out] tuple The integer tuple indicating the number of rows inserted
   * into the table
   * @param[out] rid The next tuple RID produced by the insert (ignore, not
   * used)
   * @return `true` if a tuple was produced, `false` if there are no more tuples
   *
   * NOTE: InsertExecutor::Next() does not use the `rid` out-parameter.
   * NOTE: InsertExecutor::Next() returns true with number of inserted rows
   * produced only once.
   */
