...
auto Insert(Transaction *txn, BustubInstance &instance, int v1) -> void {
  ...
  std::string sql = fmt::format(
      "INSERT INTO t1 VALUES ({}, 1), ({}, 2), ({}, 3)", v1, v1, v1);
...
auto ExpectResult(const std::string &actual_result,
                  const std::string &expected_result) -> bool {
...
auto Scan(Transaction *txn, BustubInstance &instance,
          const std::vector<int> &v1) -> void {
  std::stringstream ss;
  auto writer = bustub::SimpleStreamWriter(ss, true, ",");
...
auto GetDbForVisibilityTest(const std::string &name)
    -> std::shared_ptr<BustubInstance> {
  ...
  instance->ExecuteSql("INSERT INTO t1 VALUES (233, 1), (233, 2), (233, 3), "
                       "(234, 1), (234, 2), (234, 3)",
                       writer);
  return instance;
}

auto GetDbForCommitAbortTest(const std::string &name)
    -> std::shared_ptr<BustubInstance> {
  auto instance = std::make_unique<BustubInstance>();
  auto writer = bustub::SimpleStreamWriter(std::cout, true);
  fmt::print(stderr, "--- TEST CASE {} ---\n", name);
...
auto GetDbForIsolationTest() -> std::shared_ptr<BustubInstance> {
  ...
  // in case you implemented scan filter pushdown... disable it by forcing
  // starter rule!
  ...
  instance->ExecuteSql(
      "INSERT INTO t1 VALUES (1, 2), (1, 3), (1, 4), (2, 2), (2, 3), (2, 4);",
      writer);
  return instance;
}

auto StartReadTxn(IsolationLevel read_txn_level, BustubInstance &instance,
                  bool read_before_write) -> Transaction * {
  ...
  if (read_before_write) {
    ...
    auto is_eq =
        ExpectResult(result.str(), "1,2,\n1,3,\n1,4,\n2,2,\n2,3,\n2,4,\n");
    EXPECT_TRUE(is_eq);
    fmt::print(stderr, "1.3: read done\n");
    if (!is_eq) {
      fmt::print(stderr, "ERROR: initial read failed, force abort, you may see "
                         "leak sanitizer warning and you can ignore "
                         "it\n");
      return nullptr;
    }
  }
...
auto StartWriteTxn(IsolationLevel write_txn_level, BustubInstance &instance,
                   bool is_delete) -> Transaction * {
  auto writer = bustub::SimpleStreamWriter(std::cout, true);
  ...
  if (is_delete) {
    ...
    instance.ExecuteSqlTxn("INSERT INTO t1 VALUES (233, 1), (233, 2), (233, 3)",
                           writer, txn_w);
  }
...
auto TryRead(Transaction *txn_r, BustubInstance &instance, ExpectedOutcome o,
             bool is_delete, std::atomic<bool> &check_done) -> bool {
  bool ret = false;
  std::stringstream result;
  auto writer = bustub::SimpleStreamWriter(result, true, ",");
  ...
  case ExpectedOutcome::BlockOnRead:
    if (!check_done.load(std::memory_order_seq_cst)) {
      fmt::print(stderr,
                 "ERROR: read txn should BlockOnRead, it reads instead\n");
...
void ExpectTwoTxn(const std::string &test_name, IsolationLevel read_txn_level,
                  IsolationLevel write_txn_level, bool read_before_write,
                  bool is_delete, ExpectedOutcome o) {
  fmt::print(stderr, "--- {} ---\n", test_name);
  ...
  std::thread thread_may_block(
      [instance, txn_r, o, &check_done, write_txn_level,
       is_delete](std::promise<bool> succeed) {
        if (o != ExpectedOutcome::BlockOnWrite) {
          succeed.set_value(
              TryRead(txn_r, *instance, o, is_delete, check_done));
  ...
  case ExpectedOutcome::Read:
    ...
    break;
  case ExpectedOutcome::BlockOnRead:
    // TODO(student): if you are using a debugger, you probably want to
    // uncomment this line. future.wait();
