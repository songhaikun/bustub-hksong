...
auto BooleanType::CompareEquals(const Value &left, const Value &right) const
    -> CmpBool {
...
auto BooleanType::CompareNotEquals(const Value &left, const Value &right) const
    -> CmpBool {
  assert(GetTypeId() == TypeId::BOOLEAN);
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto BooleanType::CompareLessThan(const Value &left, const Value &right) const
    -> CmpBool {
  assert(GetTypeId() == TypeId::BOOLEAN);
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto BooleanType::CompareLessThanEquals(const Value &left,
...
auto BooleanType::CompareGreaterThan(const Value &left,
                                     const Value &right) const -> CmpBool {
  assert(GetTypeId() == TypeId::BOOLEAN);
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto BooleanType::CompareGreaterThanEquals(const Value &left,
...
auto BooleanType::Copy(const Value &val) const -> Value {
  return {TypeId::BOOLEAN, val.value_.boolean_};
}

auto BooleanType::CastAs(const Value &val, const TypeId type_id) const
    -> Value {
  switch (type_id) {
  case TypeId::BOOLEAN:
    return Copy(val);
