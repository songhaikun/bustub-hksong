...
#define TINYINT_MODIFY_FUNC(METHOD, OP)                                        \
  ...
  case TypeId::DECIMAL:                                                        \
    return Value(TypeId::DECIMAL,                                              \
                 left.value_.tinyint_ OP right.GetAs<double>());               \
  case TypeId::VARCHAR: {                                                      \
    auto r_value = right.CastAs(TypeId::TINYINT);                              \
    /* NOLINTNEXTLINE  */                                                      \
...
auto TinyintType::IsZero(const Value &val) const -> bool {
...
auto TinyintType::Modulo(const Value &left, const Value &right) const -> Value {
  ...
  case TypeId::DECIMAL:
    return {TypeId::DECIMAL,
            ValMod(left.value_.tinyint_, right.GetAs<double>())};
  case TypeId::VARCHAR: {
    auto r_value = right.CastAs(TypeId::TINYINT);
...
auto TinyintType::OperateNull(const Value &left __attribute__((unused)),
...
auto TinyintType::CompareEquals(const Value &left, const Value &right) const
    -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
...
auto TinyintType::CompareNotEquals(const Value &left, const Value &right) const
    -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto TinyintType::CompareLessThan(const Value &left, const Value &right) const
    -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto TinyintType::CompareLessThanEquals(const Value &left,
...
auto TinyintType::CompareGreaterThan(const Value &left,
                                     const Value &right) const -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto TinyintType::CompareGreaterThanEquals(const Value &left,
...
auto TinyintType::CastAs(const Value &val, const TypeId type_id) const
    -> Value {
  switch (type_id) {
  case TypeId::TINYINT: {
    if (val.IsNull()) {
