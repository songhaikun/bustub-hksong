...
#define SMALLINT_MODIFY_FUNC(METHOD, OP)                                       \
  ...
  case TypeId::DECIMAL:                                                        \
    return Value(TypeId::DECIMAL,                                              \
                 left.value_.smallint_ OP right.GetAs<double>());              \
  case TypeId::VARCHAR: {                                                      \
    auto r_value = right.CastAs(TypeId::SMALLINT);                             \
    /* NOLINTNEXTLINE */                                                       \
...
auto SmallintType::IsZero(const Value &val) const -> bool {
...
auto SmallintType::Modulo(const Value &left, const Value &right) const
  ...
  case TypeId::DECIMAL:
    return {TypeId::DECIMAL,
            ValMod(left.value_.smallint_, right.GetAs<double>())};
  case TypeId::VARCHAR: {
    auto r_value = right.CastAs(TypeId::SMALLINT);
...
auto SmallintType::OperateNull(const Value &left __attribute__((unused)),
...
auto SmallintType::CompareEquals(const Value &left, const Value &right) const
    -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
...
auto SmallintType::CompareNotEquals(const Value &left, const Value &right) const
    -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto SmallintType::CompareLessThan(const Value &left, const Value &right) const
    -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto SmallintType::CompareLessThanEquals(const Value &left,
...
auto SmallintType::CompareGreaterThan(const Value &left,
                                      const Value &right) const -> CmpBool {
  assert(left.CheckInteger());
  assert(left.CheckComparable(right));
  if (left.IsNull() || right.IsNull()) {
...
auto SmallintType::CompareGreaterThanEquals(const Value &left,
...
auto SmallintType::CastAs(const Value &val, const TypeId type_id) const
    -> Value {
  switch (type_id) {
  case TypeId::TINYINT: {
    if (val.IsNull()) {
    ...
    if (val.GetAs<int16_t>() > BUSTUB_INT8_MAX ||
        val.GetAs<int16_t>() < BUSTUB_INT8_MIN) {
