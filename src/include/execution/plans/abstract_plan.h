...
#define BUSTUB_PLAN_NODE_CLONE_WITH_CHILDREN(cname)                            \
  auto CloneWithChildren(std::vector<AbstractPlanNodeRef> children)            \
      const->std::unique_ptr<AbstractPlanNode>                                 \
          override {                                                           \
...
public:
  ...
  auto GetChildAt(uint32_t child_idx) const -> AbstractPlanNodeRef {
  ...
  auto ToString(bool with_schema = true) const -> std::string {
    if (with_schema) {
      return fmt::format("{} | {}{}", PlanNodeToString(), output_schema_,
                         ChildrenToString(2, with_schema));
    }
    return fmt::format("{}{}", PlanNodeToString(),
                       ChildrenToString(2, with_schema));
  }
...
protected:
  ...
  auto ChildrenToString(int indent, bool with_schema = true) const
      -> std::string;
...
struct fmt::formatter<
    T,
    std::enable_if_t<std::is_base_of<bustub::AbstractPlanNode, T>::value, char>>
