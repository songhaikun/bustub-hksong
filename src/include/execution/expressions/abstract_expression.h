...
#define BUSTUB_EXPR_CLONE_WITH_CHILDREN(cname)                                 \
  auto CloneWithChildren(std::vector<AbstractExpressionRef> children)          \
      const->std::unique_ptr<AbstractExpression>                               \
          override {                                                           \
...
public:
   ...
   */
  virtual auto Evaluate(const Tuple *tuple, const Schema &schema) const
      -> Value = 0;
  ...
  auto GetChildren() const -> const std::vector<AbstractExpressionRef> & {
    return children_;
  }
...
struct fmt::formatter<
    T, std::enable_if_t<std::is_base_of<bustub::AbstractExpression, T>::value,
...
struct fmt::formatter<
    std::unique_ptr<T>,
    std::enable_if_t<std::is_base_of<bustub::AbstractExpression, T>::value,
                     char>> : fmt::formatter<std::string> {
