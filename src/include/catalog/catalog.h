...
struct IndexInfo {
  ...
  IndexInfo(Schema key_schema, std::string name, std::unique_ptr<Index> &&index,
      ...
      : key_schema_{std::move(key_schema)}, name_{std::move(name)},
        index_{std::move(index)}, index_oid_{index_oid},
        table_name_{std::move(table_name)}, key_size_{key_size} {}
...
public:
  ...
  auto CreateTable(Transaction *txn, const std::string &table_name,
    ...
    // TODO(Wan,chi): This should be refactored into a private ctor for the
    // binder tests, we shouldn't allow nullptr. When create_table_heap ==
    // false, it means that we're running binder tests (where no txn will be
    // provided) or we are running shell without buffer pool. We don't need to
    // create TableHeap in this case.
  ...
  auto CreateIndex(Transaction *txn, const std::string &index_name,
    ...
    for (auto iter = table_meta->table_->MakeIterator(); !iter.IsEnd();
         ++iter) {
    ...
    // itself
    auto index_info =
        std::make_unique<IndexInfo>(key_schema, index_name, std::move(index),
