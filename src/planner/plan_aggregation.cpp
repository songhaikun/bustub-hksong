...
auto Planner::PlanSelectAgg(const SelectStatement &statement,
   ...
   * Now that we have,
   * ```
   * select v3, max(v1) + max(v2) from table where <cond> group by v3 having
   * count(v4) > count(v5);
   * ```
   *
   * Before this comment section, we have `from table where <cond>` planned as
   * filter / table scan.
   *
   * Given this is a group-by query, we will have something like this in the
   * final plan,
   * ```
   * <Parent Plan Nodes>
   *   Projection v3, max(v1) + max(v2)
   *     Filter count(v4) > count(v5)
   *       Aggregation group_by=v3 agg_types=[max, max, count, count]
   * agg_expr=[v1, v2, v4, v5] <Filter / Table Scan> --- "table scan child"
   * ```
   *
   * For every expression in select list and having clause, we will do a
   * two-phase planning. Firstly, we plan all aggregation calls, and then we
   * plan other expressions. Let's take `max(v1) + max(v2)` as an example.
   *
   * - We plan `max(v1)` and `max(v2)` in the select list and `count(v4)`,
   * `count(v5)` using the "table scan child". We get tuples of `[agg_type,
   * abstract_expression]`.
   * - Then we construct the aggregation plan node with all required
   * aggregations plus group-by expressions.
   * - After that we plan the filter `count(v4) > count(v5)` and the projection
   * `v3`, `max(v1) + max(v2)`.
   * - That's all!
   */
  ...
  // Phase-1: plan an aggregation plan node out of all of the information we
  // have.
