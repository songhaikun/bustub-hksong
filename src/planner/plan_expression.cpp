...
auto Planner::PlanBinaryOp(const BoundBinaryOp &expr,
  ...
  return GetBinaryExpressionFromFactory(op_name, std::move(left),
                                        std::move(right));
}
...
auto Planner::PlanColumnRef(const BoundColumnRef &expr,
  ...
  if (children.size() == 1) {
    ...
    // Before we can call `schema.GetColIdx`,  we need to ensure there's no
    // duplicated column.
    bool found = false;
  ...
  if (children.size() == 2) {
     ...
     *   SeqScan col1, col2
     * ```
     * In `ColumnRef n.m`, when executor is using the expression, it picks from
     * its nth children's mth column to get the data.
     */
...
void Planner::AddAggCallToContext(BoundExpression &expr) {
  ...
  case ExpressionType::AGG_CALL: {
    ...
    auto agg_call =
        BoundAggCall(agg_name, agg_call_expr.is_distinct_,
                     std::vector<std::unique_ptr<BoundExpression>>{});
    // Replace the agg call in the original bound expression with a pseudo one,
    // add agg call to the context.
    ctx_.AddAggregation(std::make_unique<BoundAggCall>(
        std::exchange(agg_call_expr, std::move(agg_call))));
    return;
  }
  ...
  throw Exception(fmt::format("expression type {} not supported in planner yet",
                              expr.type_));
}
...
auto Planner::PlanExpression(const BoundExpression &expr,
  ...
  case ExpressionType::AGG_CALL: {
    ...
    return std::make_tuple(
        UNNAMED_COLUMN, std::move(ctx_.expr_in_agg_[ctx_.next_aggregation_++]));
  }
  case ExpressionType::COLUMN_REF: {
  ...
  case ExpressionType::FUNC_CALL: {
    ...
    return std::make_tuple(UNNAMED_COLUMN,
                           PlanFuncCall(func_call_expr, children));
  }
  case ExpressionType::CONSTANT: {
