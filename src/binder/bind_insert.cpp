...
auto Binder::BindInsert(duckdb_libpgquery::PGInsertStmt *pg_stmt)
  ...
  if (pg_stmt->cols != nullptr) {
    throw NotImplementedException(
        "insert only supports all columns, don't specify columns");
  }
  ...
  auto select_statement = BindSelect(
      reinterpret_cast<duckdb_libpgquery::PGSelectStmt *>(pg_stmt->selectStmt));

  return std::make_unique<InsertStatement>(std::move(table),
                                           std::move(select_statement));
}

auto Binder::BindDelete(duckdb_libpgquery::PGDeleteStmt *stmt)
...
auto Binder::BindUpdate(duckdb_libpgquery::PGUpdateStmt *stmt)
  ...
  if (stmt->withClause != nullptr) {
    throw bustub::NotImplementedException(
        "update with clause not supported yet");
  }

  if (stmt->fromClause != nullptr) {
    throw bustub::NotImplementedException(
        "update from clause not supported yet");
  }
  ...
  for (auto cell = root->head; cell != nullptr; cell = cell->next) {
    auto target = reinterpret_cast<duckdb_libpgquery::PGResTarget *>(
        cell->data.ptr_value);
    auto column = ResolveColumnRefFromBaseTableRef(
        *table, std::vector{std::string{target->name}});
    target_expr.emplace_back(
        std::make_pair(std::move(column), BindExpression(target->val)));
  }
