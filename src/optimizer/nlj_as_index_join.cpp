  ...
  for (const auto *index_info : catalog_.GetTableIndexes(table_name)) {
    if (key_attrs == index_info->index_->GetKeyAttrs()) {
      return std::make_optional(
          std::make_tuple(index_info->index_oid_, index_info->name_));
    }
  }
  return std::nullopt;
}

auto Optimizer::OptimizeNLJAsIndexJoin(const AbstractPlanNodeRef &plan)
    -> AbstractPlanNodeRef {
  ...
  if (optimized_plan->GetType() == PlanType::NestedLoopJoin) {
    ...
    BUSTUB_ENSURE(nlj_plan.children_.size() == 2,
                  "NLJ should have exactly 2 children.");
    // Check if expr is equal condition where one is for the left table, and one
    // is for the right table.
    if (const auto *expr = dynamic_cast<const ComparisonExpression *>(
            nlj_plan.Predicate().get());
        expr != nullptr) {
              ...
              if (left_expr->GetTupleIdx() == 0 &&
                ...
                if (auto index = MatchIndex(right_seq_scan.table_name_,
                  ...
                  return std::make_shared<NestedIndexJoinPlanNode>(
                      ...
                      right_seq_scan.GetTableOid(), index_oid,
