...
auto Optimizer::OptimizeMergeProjection(const AbstractPlanNodeRef &plan)
    -> AbstractPlanNodeRef {
  ...
  if (optimized_plan->GetType() == PlanType::Projection) {
    ...
    BUSTUB_ENSURE(optimized_plan->children_.size() == 1,
                  "Projection with multiple children?? That's weird!");
    ...
    if (std::equal(child_columns.begin(), child_columns.end(),
      ...
      for (size_t idx = 0; idx < exprs.size(); idx++) {
        auto column_value_expr =
            dynamic_cast<const ColumnValueExpression *>(exprs[idx].get());
